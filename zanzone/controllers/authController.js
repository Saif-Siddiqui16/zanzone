...
exports.publicSignup = async (req, res) => {
    try {
        ...
        // ── PERSONAL ACCOUNT ─────────────────────────────────────────────────
        // Default company ID = 1 (HQ), instant activation, role = customer
        if (accountType === 'personal') {
            const hqCompanyId = 1;
            const [result] = await db.query(
                `INSERT INTO users (company_id, name, email, password, phone, role, status)
                 VALUES (?, ?, ?, ?, ?, 'customer', 'active')`,
                [hqCompanyId, name.trim(), cleanEmail, hashedPassword, phone || null]
            );

            // Also insert into customers table immediately so they are mapped to the HQ/Default Company and visible to admins!
            await db.query(
                `INSERT INTO customers (company_id, name, email, phone, client_type, status, created_by)
                 VALUES (?, ?, ?, ?, 'Direct', 'active', ?)`,
                [hqCompanyId, name.trim(), cleanEmail, phone || null, result.insertId]
            );
