...
exports.getAll = async (req, res) => {
  try {
    ...
    const fallbackCompanyId = normalizePositiveInt(
    ...
    if (!companyId) {
      const [anyCompany] = await db.query(
  ...
  try {
    ...
    if (!companyId) {
      return errorResponse(
        res,
        "Valid company_id is required to create vendor.",
        400,
      );
    }
    ...
    if (!companyRows.length && isPlatformAdmin) {
      ...
      if (allCompanies.length === 0) {
        console.log(
          "Emergency: No companies found in Vendor Create. Creating HQ Company.",
        );
    ...
    if (!companyRows.length) {
      return errorResponse(
        res,
        "Invalid company_id. Company not found. Please create a company first.",
        400,
      );
    }
    ...
    const roleNormPost = String(req.user?.role || "")
      .toLowerCase()
      .trim()
      .replace(/\s+/g, "_");
    // Only super admin may auto-activate vendor accounts. Other roles must request approval.
    const canAutoActivate = ["super_admin"].includes(roleNormPost);
    ...
    return successResponse(
      res,
      { id: result.insertId, name: String(name).trim(), status: safeStatus },
      "Vendor created.",
      201,
    );
  } catch (err) {
    ...
    return errorResponse(
      res,
      `Failed to create vendor. ${err.sqlMessage || err.message || ""}`.trim(),
      500,
    );
  }
};
...
exports.update = async (req, res) => {
  try {
      ...
      if (key === "status") {
        ...
        if (!isSuperAdminRole(req.user?.role)) {
          ...
          if (statusNorm === "active" || statusNorm === "blacklisted") {
            return errorResponse(
              res,
              "Only super admin can approve or blacklist vendors.",
              403,
            );
          }
    ...
    await db.query(
      `UPDATE vendors SET ${sets.join(", ")} WHERE id = ?${cs.clause}`,
      values,
    );
...
exports.remove = async (req, res) => {
  try {
    ...
    await db.query(`DELETE FROM vendors WHERE id = ?${cs.clause}`, [
      req.params.id,
      ...cs.params,
    ]);
