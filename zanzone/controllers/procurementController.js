...
exports.getPOs = async (req, res) => {
  try {
    ...
    const [rows] = await db.query(
      `SELECT po.*, v.name as vendor_name FROM purchase_orders po LEFT JOIN vendors v ON po.vendor_id = v.id WHERE 1=1 ${cf.clause} ORDER BY po.created_at DESC`,
      cf.params,
    );
    return successResponse(res, rows);
  } catch (err) {
  ...
  try {
    ...
    for (const received of parsedReceivedItems) {
      ...
      if (item) {
        ...
        if (item.received_qty < (item.quantity || item.orderedQty))
          allReceived = false;
