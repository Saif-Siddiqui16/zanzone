...
exports.createEvent = async (req, res) => {
    try {
        const { name, event_date, location, client_id, manager_id, status, special_requests, planner_name, guest_count, mood_board_url } = req.body;
        ...
        const [result] = await db.query(
            ...
            [companyId, name, event_date || null, location || null, cleanClientId, manager_id || req.user.id, finalStatus, imageUrl, special_requests || null, planner_name || null, guest_count || 0, mood_board_url || null]
        );
...
exports.updateEvent = async (req, res) => {
    try {
        const { name, event_date, location, client_id, status, special_requests, planner_name, guest_count, mood_board_url } = req.body;
        ...
        const [result] = await db.query(
            `UPDATE events SET name = COALESCE(?, name), event_date = COALESCE(?, event_date), location = COALESCE(?, location), client_id = COALESCE(?, client_id), status = COALESCE(?, status), image_url = COALESCE(?, image_url), special_requests = COALESCE(?, special_requests), planner_name = COALESCE(?, planner_name), guest_count = COALESCE(?, guest_count), mood_board_url = COALESCE(?, mood_board_url) WHERE id = ?${cs.clause}`,
            [name, event_date, location, cleanClientId, dbStatus, imageUrl, special_requests, planner_name, guest_count, mood_board_url, req.params.id, ...cs.params]
        );

        // If no rows were affected, the record was not updated (not found or out-of-scope)
        if (!result || result.affectedRows === 0) {
            return errorResponse(res, 'Event not found or no permission to update.', 404);
        }
