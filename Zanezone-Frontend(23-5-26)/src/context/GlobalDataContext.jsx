...
export const GlobalDataProvider = ({ children }) => {
    ...
    const filterDataForCurrentUser = React.useCallback((dataArray) => {
        ...
        const myCompanyId = currentUser.company_id || currentUser.companyId || currentUser.clientId || currentUser.client_id;
        const myUserId = currentUser.id;

        // If the user has a specific company scope, filter by that company
        if (myCompanyId && String(myCompanyId) !== '1') {
            return dataArray.filter(item => {
                const itemCompany = item.company_id || item.companyId || item.client_id || item.clientId || item.customer_id;
                const itemUser = item.user_id || item.userId || item.created_by || item.requested_by || item.manager_id || item.submitted_by;

                // If it belongs to their company OR if they created it
                return (itemCompany && String(itemCompany) === String(myCompanyId)) ||
        ...
        if (role === 'customer') {
            return dataArray.filter(item => {
                const itemCompany = item.company_id || item.companyId || item.client_id || item.clientId || item.customer_id;
                const itemUser = item.user_id || item.userId || item.created_by || item.requested_by || item.manager_id || item.submitted_by;
                return (itemUser && String(itemUser) === String(myUserId)) ||
    ...
    const cancelPersonalMembership = React.useCallback(async (targetUserId) => {
        try {
            const next = {
                ...currentUser,
    ...
    const addEvent = async (event) => {
        try {
            const rawClientId = event.client_id || clients.find(c => c.name === event.client)?.id || null;
            const clientId = rawClientId ? String(rawClientId).replace('CLT-', '') : '';
    ...
    const updateEvent = async (updated) => {
        try {
            ...
            if (updated.client_id !== undefined) {
                ...
                const found = clients.find(c => c.name === updated.client)?.id || null;
                clientId = found ? String(found).replace('CLT-', '') : undefined;
            }

            const formData = new FormData();
            if (updated.title !== undefined) formData.append('name', updated.title);
            if (updated.date !== undefined) formData.append('event_date', updated.date);
            if (updated.location !== undefined) formData.append('location', updated.location);
            ...
            if (updated.specialRequests !== undefined) formData.append('special_requests', updated.specialRequests);
            if (updated.plannerName !== undefined) formData.append('planner_name', updated.plannerName);
            if (updated.guestCount !== undefined) formData.append('guest_count', updated.guestCount);
            else if (updated.guests !== undefined) formData.append('guest_count', updated.guests);
            if (updated.moodBoardUrl !== undefined) formData.append('mood_board_url', updated.moodBoardUrl);
            ...
            try {
                ...
                console.debug(`PUT /support/events/${updated.id} formData:`, entries, 'response:', res?.data || res);
            ...
            if (updated.plannerName !== undefined) uiUpdate.plannerName = updated.plannerName;
            if (updated.specialRequests !== undefined) uiUpdate.specialRequests = updated.specialRequests;
            if (updated.guestCount !== undefined) uiUpdate.guestCount = updated.guestCount;
            if (updated.moodBoardUrl !== undefined) uiUpdate.moodBoardUrl = updated.moodBoardUrl;

            if (Object.keys(uiUpdate).length > 0) {
                setEvents(prev => prev.map(e => (String(e.id) === String(updated.id) ? { ...e, ...uiUpdate } : e)));
            }

            await fetchTickets();
            addLog({ action: 'Event Update', detail: `Synchronized details for ${updated.title || updated.id}.`, type: 'system' });
        } catch (error) {
    ...
    return (
        <GlobalDataContext.Provider value={{
            // Auth & User
            currentUser, setCurrentUser, activatePersonalMembership, cancelPersonalMembership, hasPermission, menuPermissions, setMenuPermissions, hasMenuPermission, roles: ['super_admin', 'operations', 'procurement', 'inventory', 'logistics', 'finance', 'sales', 'support'],
